...
Analyzing income distribution and wealth concentration helps us identify counties with strong economic foundations and assess their crisis resilience. This, in combination with COVID-19 case and death data, will guide the stakeholder in making an informed decision on where to invest in developing a mixed-use building. Counties that maintained steady consumer traffic and economic activity during the pandemic are more likely to support stable and sustainable business growth.
...
### K-Means Clustering
The K-Means clustering plot, Figure 1, shows how Texas counties are grouped into two distinct clusters (1 and 2). Each point represents a county, and clusters are distinguished by unique shapes and colors for clear visualization. The boarder around each cluster provides a visual boundary for each group. This clustering helps uncover patterns among the counties based on their economic resilience during the COVID-19 pandemic. 
...
Cluster 1 has a high concentration of counties (about 95%) while Cluster 2 captures a much smaller group. The imbalance questions the effectiveness of the clustering approach in capturing meaningful economic and pandemic-related differences across Texas counties. The observed patterns show some differences between clusters, but they may be a result of population size rather than distinct economic or pandemic-related characteristics.
...
The choice of this feature thus helps explore the correlation between economic factors and the severity of the pandemic's impact, offering critical and clear insights into the resilience and vulnerabilities of different counties. The **contingency table** below, Table 4, compares the clusters generated by K-means with the death-to-case ratio categories, allowing us to evaluate if the clustering effectively captures differences in pandemic outcomes associated with economic conditions.
...
A summary statistics table, Table 5, similar to the previous clustering method, is used to provide a detailed breakdown of the average values for key features across the two supervised clusters identified through K-Means clustering. Each cluster represents a distinct group of Texas counties with more similar economic, demographic, and pandemic characteristics. The table displays the **average median income**, **income per capita**, **rent burden levels** (both for households spending more than 50% and 30-35% of their income on rent), **confirmed COVID-19 cases**, **deaths**, and **total population** for each cluster. These statistics help in interpreting the composition and characteristics of the counties in each group, making it easier to assess economic and demographic differences. 
...
The supervised and unsupervised K-Means clustering analyses both categorize Texas counties using economic and pandemic-related features; however, they differ in clarity, precision, and interpretability. 
...
The following table, Table 7, presents the purity scores for the two different subgroup classifications: Income Groups and Rent Burden Groups. 
...
The Silhouette Method, Figure 9, evaluates how well each data point fits within its assigned cluster compared to other clusters. The Silhouette score ranges from -1 to 1, with values close to 1 meaning that the points are well-clustered. In the following Silhouette chart, the peak occurs at 2 clusters. 
...
silhouette_scores <- purrr::map(2:6, function(k) {
  clusters <- cutree(hc_complete, k = k)
  sil <- silhouette(clusters, distance_matrix)
  if (!is.null(sil)) mean(sil[, 3]) else NA  # Calculate average silhouette width or NA
})

# Plot silhouette scores
silhouette_data <- data.frame(
  k = 2:6,
  avg_silhouette_width = unlist(silhouette_scores)
)

ggplot(silhouette_data, aes(x = k, y = avg_silhouette_width)) +
  geom_point() +
  geom_line() +
  labs(title = "Average Silhouette Width for Different Cluster Counts",
       x = "Number of Clusters k",
       y = "Average Silhouette Width")
...
The following dendrograms, Figure 11, represent hierarchical clustering using two additional linkage methods: Average Linkage and Ward’s Linkage. Each method groups Texas counties based on the similarities in economic and pandemic-related features.
...
\vspace{5pt}
  ...
  - Mortality rates are a key public health indicator that directly reflects the severity of the pandemic's impact, making them essential for assessing a county's resilience. This feature provides valuable insights into how a county’s income level might influence its resilience and lower mortality rates during a pandemic like COVID-19. 
  
\vspace{5pt}

The choice of this feature thus helps explore the correlation between economic factors and the severity of the pandemic's impact, offering critical and clear insights into the resilience and vulnerabilities of different counties. This can be seen in Figure 12.
...
#### Supervised Evaluation
The supervised hierarchical clustering analysis, Figure 13, uses the Ward’s linkage method to cluster Texas counties based on two key features: the death case ratio (COVID-19 deaths to confirmed cases) and income per capita. This method minimizes the variance within each cluster, resulting in more compact and well-separated groups. Although previously, the Average linkage was recommended, Ward's method provided the most balanced clustering which is why it was chosen for this analysis. Ward's method had a competative silhouette value, so it can be chosen as the linkage method with confidence. 
...
A summary statistics table, Table 10, similar to the previous clustering method, is used to provide a detailed breakdown of the average values for key features across the two supervised clusters identified through Hierarchical clustering. Each cluster represents a distinct group of Texas counties with more similar economic, demographic, and pandemic characteristics. The table displays the average median income, income per capita, rent burden levels (both for households spending more than 50% and 30-35% of their income on rent), confirmed COVID-19 cases, deaths, and total population for each cluster. 
...
The hierarchical clustering dendrogram with subgroup colors, Figure 14, provides a comprehensive visualization of how Texas counties are grouped based on two key characteristics: income levels and rent burden. 
...
The purity scores, Table 12, for the hierarchical clustering analysis reveal insights into how well the clusters correspond to the true labels for Income Groups and Rent Burden Groups.
...
# Log-transform population and remove NA values
data_selected <- data %>%
  mutate(log_total_pop = log1p(total_pop)) %>%
  mutate(deaths_per_k = deaths / (total_pop / 1000)) %>%
  na.omit()  # Remove rows with NA values
```

### Gaussian Mixture Models
To deepen our analysis and provide a more sophisticated clustering approach, we utilized **Gaussian Mixture Models (GMM)** to identify patterns in the data that may not be captured by simpler methods like K-Means.
...
The GMM clustering can be seen in Figure 15. The key variables used in this clustering are:
...
gmm_result1 <- Mclust(data_selected %>% select(log_total_pop, death_case_ratio))
data_selected$gmm_cluster1 <- as.factor(gmm_result1$classification)

# Count points per cluster for legend labels
cluster_counts <- data_selected %>%
  group_by(gmm_cluster1) %>%
  summarize(count = n())

# Merge counts for updated legend labels
data_selected <- data_selected %>%
  left_join(cluster_counts, by = "gmm_cluster1")

# Representative counties for labeling
representative_counties <- data_selected %>%
  filter(county_name %in% c("Dallas County", "Harris County", "Travis County")) 

# Plot GMM Clustering
ggplot(data_selected, aes(x = log_total_pop, y = death_case_ratio, color = gmm_cluster1)) +
  geom_point(alpha = 0.6, size = 2) +
  geom_smooth(method = "loess", se = FALSE, aes(group = gmm_cluster1), linetype = "dashed") +
  geom_text(
    data = representative_counties,
  ...
  scale_color_manual(
    ...
    labels = paste("Cluster", cluster_counts$gmm_cluster1, " (n=", cluster_counts$count, ")", sep = "")
  ) +
  labs(
    title = "GMM Clustering on Log(Population) and Death Case Ratio",
    x = "Log(Total Population + 1)",
    y = "Death Case Ratio",
    color = "Cluster"
  ) +
  theme_minimal() +
...
```
\captionof{figure}{GMM Clustering on Log(Population) and Death Case Ratio}

\vspace{10pt}

```{r}
format_silhouette_summary <- function(silhouette_summary) {
  # Extract cluster sizes and average silhouette widths as vectors
  cluster_ids <- as.numeric(names(silhouette_summary$clus.sizes))
  cluster_sizes <- as.numeric(unname(silhouette_summary$clus.sizes))
  avg_silhouette_widths <- as.numeric(unname(silhouette_summary$clus.avg.widths))
  
  # Create a clean data frame
  df <- data.frame(
    Cluster = cluster_ids,
    Size = cluster_sizes,
    `Avg Silhouette Width` = round(avg_silhouette_widths, 3)
  )
...
# Calculate and summarize Silhouette score for GMM Clustering 1
gmm_dist1 <- dist(data_selected %>% select(log_total_pop, death_case_ratio))
gmm_silhouette1 <- silhouette(as.integer(data_selected$gmm_cluster1), gmm_dist1)
gmm_silhouette_summary1 <- summary(gmm_silhouette1)

# Convert to data frame for display
gmm_silhouette_df1 <- format_silhouette_summary(gmm_silhouette_summary1)

# Display Silhouette Summary for GMM Clustering 1
knitr::kable(
  gmm_silhouette_df1, 
  caption = "GMM Clustering Silhouette Scores on Log-Transformed Population and Death Case Ratio", 
  format = "latex",
  booktabs = TRUE,
  position = "H"
)
...
The clusterings appear well-defined. Clusters 1 and 3 have lower death case ratios than Cluster 2, on average. Cluster 2 represents counties within a relatively small range of intermediate population sizes that experienced moderate to high death ratios. These are notable for our analysis as these counties are unlikely to demonstrate the heath resilience desired by our stakeholder. 

\vspace{5pt}

Based on the silhouette widths, seen in Table 13, we note that none of these clusters is particularly well-defined. Even the most well-defined cluster (2) exhibits a measly 0.177, indicating low cohesion in the cluster. These silhouette scores suggest suboptimal clustering, indicating that the selected features may not be able to provide the necessary information for distinguishing between counties without additional information. 
...
The clustering provides insight into population size and health outcomes in Texas counties. This insight can be see in table 14. The first cluster houses low population counties. These counties performed the best in terms of health outcomes with the lowest death to case ratio (0.01). These are likely rural counties with low population density and lower health risk factors. The second cluster has a higher population and the worst health outcomes of any cluster. This is interesting because an intuitive explanation suggests that more populous counties expedience worse health outcomes, and this clustering contradicts that intuition. The third and final cluster is the largest cluster. Counties in cluster three have a higher population on average than either of the other two clusters. Health outcomes in cluster three (death to case ratio of 0.02) are worse than those in cluster 1 but better than those in cluster 2. Cluster 3 likely encompasses urban counties where the risk factors are lower despite high population density.    

### GMM Clustering on Median Income and Mortality Rate
The second GMM clustering, below, focuses on **Income** and **Mortality Rate** as defined by deaths per one thousand residents. Using these features, the **mclust** function determined that two clusters is appropriate to maximize the average silhouette width across clusters. This can be seen in Figure 17. 

\vspace{10pt}

```{r, eval=TRUE}
# Apply GMM clustering for "GMM Clustering on Median Income and Mortality Rate"
gmm_result2 <- Mclust(data_selected %>% select(deaths_per_k, median_income, income_per_capita))
data_selected$gmm_cluster2 <- as.factor(gmm_result2$classification)
...
# Update legend labels with cluster counts
data_selected <- data_selected %>%
  left_join(cluster_counts, by = "gmm_cluster2")

# Plot the clustering
ggplot(data_selected, aes(x = median_income, y = deaths_per_k, color = gmm_cluster2)) +
  geom_point(alpha = 0.6, size = 2) +
  geom_smooth(method = "lm", se = FALSE, aes(group = gmm_cluster2), linetype = "dashed") +
  scale_color_manual(
    values = c("red", "green"),
    labels = paste("Cluster", cluster_counts$gmm_cluster2, " (n=", cluster_counts$count, ")", sep = "")
  ) +
  labs(
    title = "GMM Clustering on Income and Mortality Rate",
    x = "Median Income",
...
```
\captionof{figure}{GMM Clustering on Income and Mortality Rate}

\vspace{10pt}

```{r}
# Calculate and summarize Silhouette score for GMM Clustering 2
gmm_dist2 <- dist(data_selected %>% select(deaths_per_k, median_income, income_per_capita))
gmm_silhouette2 <- silhouette(as.integer(data_selected$gmm_cluster2), gmm_dist2)
gmm_silhouette_summary2 <- summary(gmm_silhouette2)

# Convert to data frame for display
gmm_silhouette_df2 <- format_silhouette_summary(gmm_silhouette_summary2)

# Display Silhouette Summary for GMM Clustering 2
knitr::kable(
  gmm_silhouette_df2, 
  caption = "GMM Clustering Silhouette Scores on Income and Mortality Rate", 
  format = "latex",
  booktabs = TRUE
  ) %>%
  kableExtra::kable_styling(latex_options = c("hold_position", "scale_down"))
...
Based on the GMM clustering of the deaths per thousand residents and median income, we observe two clusters with fairly low cohesion levels, seen in Table 15. The first cluster contains roughly twice as many counties as the second. Counties in cluster 1 represent lower to mid-income counties with higher mortality rates. The relatively high silhouette score of (0.44) indicates that this cluster is quite distinct, with counties that share similar qualities in terms of income and COVID mortality. The second cluster represents higher income counties with lower mortality rates. This cluster is more ill-defined than the first (indicated by its lower silhouette width of 0.02). The low silhouette width indicates that these counties are not as homogeneous and may overlap with the counties in cluster 1 in terms of both their income and mortality rate. 

### Supervised Evaluation
\vspace{10pt}
```{r graph, eval=TRUE}

# Perform PCA on relevant variables
pca_result2 <- prcomp(data_selected %>% select(deaths_per_k, median_income, income_per_capita), scale. = TRUE)

# Create a data frame with PCA results and cluster assignments
pca_df2 <- data.frame(PC1 = pca_result2$x[,1], PC2 = pca_result2$x[,2], Cluster = data_selected$gmm_cluster2)

# Plot PCA results
ggplot(pca_df2, aes(PC1, PC2, color = Cluster)) +
  geom_point(alpha = 0.7) +
  labs(title = "PCA of GMM Clusters on Median Income and Mortality Rate", x = "Principal Component 1", y = "Principal Component 2") +
  theme_minimal()

library(scales)  # For comma formatting
```
\captionof{figure}{PCA of GMM Clusters on Median Income and Mortality Rate}

\vspace{10pt}

```{r table, eval=TRUE}
# Create a table with shorter, simpler column names and formatted numbers
cluster_profiles2 <- data_selected %>%
  group_by(gmm_cluster2) %>%
  summarise(
    Count = n(),
    `Avg Deaths` = round(mean(deaths_per_k), 2),
    `Avg Income` = comma(round(mean(median_income), 2)),
    `Avg Capita Inc` = comma(round(mean(income_per_capita), 2)),
    `Med Deaths` = round(median(deaths_per_k), 2),
    `Med Income` = comma(round(median(median_income), 2)),
    `Med Capita Inc` = comma(round(median(income_per_capita), 2))
  )

# Display the table with simplified headers
knitr::kable(
  cluster_profiles2, 
  caption = "Cluster Profiles for GMM Clustering on Median Income and Mortality Rate",
  format = "latex",
  booktabs = TRUE
  ) %>%
  kableExtra::kable_styling(latex_options = c("hold_position", "scale_down"))
...
The supervised evaluation, Figure 18, plots the points using PCA to reduce the dimensionality of the data while preserving variance. The clusters from the second GMM clustering are plotted against the PCA in order to visualize the results of the clustering. Cluster 1 represents counties with a relatively higher mortality rate of 2.23 deaths per thousand, double that of cluster 2. Additionally, as seen in Table 16, incomes are lower for counties in cluster 1 on average. Based on these results, we determine that cluster 2 represents affluent counties with better health outcomes and cluster 1 represents poorer counties that fared worse during the pandemic.

\newpage 

#### Ground Truth Assessment
To evaluate the success of the GMM clustering, we divided the counties into **Urban and Rural counties** based on population and income. Counties where the log of population is greater than 10 **or** the average income is above $50,000 are considered urban. While these thresholds are admittedly arbitrary, the classification of Urban versus Rural allows us to compare the ground truth with the earlier clusterings and assess their perforamance. Below, in Figure 19, is a scatter plot of the arbitrary divide between urban and rural counties that we will use as the ground truth going forward.
\vspace{10pt}

```{r}
# Define arbitrary thresholds
log_pop_threshold <- 10  # Adjust based on data distribution
income_threshold <- 50000  # Example income threshold, adjust based on distribution

# Classify areas as Urban or Rural based on thresholds
data_selected <- data_selected %>%
  mutate(Urban_Rural = case_when(
    log_total_pop >= log_pop_threshold | median_income >= income_threshold ~ "Urban",
    TRUE ~ "Rural"
  ))

# Now set Urban_Rural as a factor with specified levels
data_selected <- data_selected %>%
  mutate(Urban_Rural = factor(Urban_Rural, levels = c("Urban", "Rural")))

# Calculate counts for Urban and Rural
urban_rural_counts <- table(data_selected$Urban_Rural)
urban_count <- urban_rural_counts["Urban"]
rural_count <- urban_rural_counts["Rural"]

# Create a customized legend with counts
ggplot(data_selected, aes(x = log_total_pop, y = median_income, color = Urban_Rural)) +
  geom_point(alpha = 0.6) +
  labs(
    title = "Urban vs Rural Classification Based on Population and Income",
    x = "Log-Transformed Population",
    y = "Median Income",
    color = "Classification"
  ) +
  scale_color_manual(
    values = c("Urban" = "blue", "Rural" = "green"),
    labels = c(paste("Urban (n=", urban_count, ")", sep = ""),
               paste("Rural (n=", rural_count, ")", sep = ""))
  ) +
  theme_minimal()

```
\captionof{figure}{Urban vs Rural Classification Based on Population and Income}


\vspace{10pt}
Comparing each of our clusterings to the ground truth of Urban versus Rural, we obtained the following purity scores. **Purity** measures the extent to which the clusters produced by each model align with the ground truth. These values can be seen in Table 17.

```{r}
# Function to calculate purity
calculate_purity <- function(cluster_labels, ground_truth_labels) {
  table <- table(cluster_labels, ground_truth_labels)
  purity <- sum(apply(table, 1, max)) / length(cluster_labels)
  return(purity)
}

# Calculate purity for GMM Clustering 1 (on Log-Transformed Population and Death Case Ratio)
purity_gmm1 <- calculate_purity(data_selected$gmm_cluster1, data_selected$Urban_Rural)

# Calculate purity for GMM Clustering 2 (on Median Income and Mortality Rate)
purity_gmm2 <- calculate_purity(data_selected$gmm_cluster2, data_selected$Urban_Rural)

# Display purity results
purity_results <- data.frame(
  Clustering = c("GMM Clustering 1", "GMM Clustering 2"),
  Purity = c(purity_gmm1, purity_gmm2)
)

knitr::kable(
  purity_results, 
  caption = "Purity Scores for GMM Clusterings Based on Urban/Rural Classification",
  format = "latex",
  booktabs = TRUE,
  position = "H"
)
...
# Recommendations
The clustering analysis using various methods, such as **K-Means**, **Hierarchical Clustering**, and **Gaussian Mixture Models (GMM)**, provides a well-rounded view of Texas counties' economic and pandemic performance. Each clustering technique identified patterns related to income, population density, and pandemic resilience, ultimately highlighting a consistent group of **"good"** clusters. For stakeholders, these insights are invaluable, as they can guide strategic decisions for business investments.

\vspace{5pt}

Below, in Table 18, is a summary of the clusters that are identified as a **"good cluster"** subgroup.  

```{r, echo=FALSE, eval = TRUE}

# Create a data frame summarizing each clustering method
clustering_summary <- data.frame(
  ...
  Number_of_Clusters = c(
    ...
    2,  # K-Means Supervised
    2,  # Hierarchical Unsupervised
    2,  # Hierarchical Supervised
    3,  # GMM Clustering 1 (Log-Transformed Population and Death Case Ratio)
    2   # GMM Clustering 2 (Median Income and Mortality Rate)
  ),
  Good_Cluster = c(
    2,  # K-Means Unsupervised
    2,  # K-Means Supervised
...
# Display the summary table
kable(clustering_summary, caption = "Summary of Clustering Methods and Optimal Clusters", format = "latex", booktabs = TRUE, position = "H") %>%
  kable_styling(full_width = FALSE, font_size = 10) %>%
  column_spec(1, width = "5cm") %>%  # Set method column width
  column_spec(2, width = "3cm") %>%  # Set number_of_clusters column width
  column_spec(3, width = "3cm")      # Set good_cluster column width

```

The identified **"good"** clusters consist of affluent counties with high population density and strong performance during the pandemic. These areas are likely to provide stable economic conditions and consistent consumer traffic, reducing the risk for businesses.

\vspace{5pt}

The following recommendations are particularly useful for **property developers** interested in investing in mixed-use developments that require reliable economic and social infrastructure.

\vspace{5pt}

After taking cluster "2" in the second layer K-means cluster, and sorting from descending order according to population density, the top counties identified. This can be seen in Table 19.

```{r}

# For K-Means Unsupervised: Good cluster is 2
data_clustered_1 <- data_clustered_1 %>%
  rename(kmeans_unsup_cluster = cluster) %>%
  mutate(kmeans_unsup_good = ifelse(kmeans_unsup_cluster == 2, TRUE, FALSE))

# For K-Means Supervised: Good cluster is 2
data_clustered_2 <- data_clustered_2 %>%
  rename(kmeans_sup_cluster = cluster) %>%
  mutate(kmeans_sup_good = ifelse(kmeans_sup_cluster == 2, TRUE, FALSE))

# For Hierarchical Clustering Unsupervised: Good cluster is 2
# (Assuming `hc_complete` is the hierarchical model object for the unsupervised method)
data_clustered_hc_unsup <- data %>%
  mutate(cluster_hc_unsup = cutree(hc_complete, k = 2)) %>%
  mutate(hier_unsup_good = ifelse(cluster_hc_unsup == 2, TRUE, FALSE)) %>%
  select(county_name, hier_unsup_good, cluster_hc_unsup)

# For Hierarchical Clustering Supervised: Good cluster is 2
# (Assuming `hc_ward` is the hierarchical model object for the supervised method)
data_clustered_hc_sup <- data %>%
  mutate(cluster_hc_sup = cutree(hc_ward, k = 2)) %>%
  mutate(hier_sup_good = ifelse(cluster_hc_sup == 2, TRUE, FALSE)) %>%
  select(county_name, hier_sup_good, cluster_hc_sup)

# For Gaussian Mixture Models (GMM) Clustering 1: Good cluster is 1 (lowest death ratio)
data_selected <- data_selected %>%
  mutate(gmm_cluster_good_1 = ifelse(gmm_cluster1 == 1, TRUE, FALSE))

# For Gaussian Mixture Models (GMM) Clustering 2: Good cluster is 2 (higher income, lower mortality)
data_selected <- data_selected %>%
  mutate(gmm_cluster_good_2 = ifelse(gmm_cluster2 == 2, TRUE, FALSE))

# Step 2: Merge "good" indicators across different clustering methods
final_clusters <- data_clustered_hc_unsup %>%
  left_join(data_clustered_hc_sup, by = "county_name") %>%
  left_join(data_clustered_1 %>% select(county_name, kmeans_unsup_good), by = "county_name") %>%
  left_join(data_clustered_2 %>% select(county_name, kmeans_sup_good), by = "county_name") %>%
  left_join(data_selected %>% select(county_name, gmm_cluster_good_1, gmm_cluster_good_2), by = "county_name")

# Step 3: Count how many methods mark each county as "good"
final_clusters <- final_clusters %>%
  rowwise() %>%
  mutate(
    good_count = sum(
      c_across(ends_with("_good")), 
      na.rm = TRUE
    )
  ) %>%
  ungroup()

# Step 4: Filter counties that are "good" in at least half of the methods (≥3 methods)
final_recommended_counties <- final_clusters %>%
  mutate(
    total_methods = rowSums(!is.na(select(., ends_with("_good"))))
  ) %>%
  filter(good_count >= 3)

# Step 5: Create a concise table for final recommendations
concise_recommendations <- final_recommended_counties %>%
  select(county_name, good_count, total_methods) %>%
  arrange(desc(good_count))  # Sort by descending order of good_count

# Step 6: Display the narrower, formatted table
kable(concise_recommendations, caption = "Recommended Counties Based on Clustering Analysis", format = "latex", booktabs = TRUE, position = "H") %>%
  kable_styling(full_width = FALSE, font_size = 10) %>%
  column_spec(1, width = "6cm", bold = TRUE) %>%         # Bold county names and set column width
  column_spec(2, width = "2cm") %>%                      # Set column width for good_count
  column_spec(3, width = "2cm") %>%                      # Set column width for total_methods
  add_header_above(c(" " = 1, "Clustering Indicators" = 2))  # Add a header row for clarity

```

The analysis identifies key counties — **Tarrant, Bexar, Dallas, Harris, and Travis** — as the most promising locations for investment. These counties were consistently marked as favorable across multiple clustering methods.

\vspace{5pt}

**Tarrant County**, **Bexar County**, and **Dallas County** all have high population densities and strong economic indicators, including higher median incomes and lower death-to-case ratios. Their robust economic activity suggests they can support a wide range of businesses, such as gyms, restaurants, and retail outlets. The recommendation is that these counties are prioritized for high-traffic, mixed-use developments that benefit from urban density and economic resilience. The presence of strong healthcare infrastructure in these counties also mitigates risk during health crises.

\vspace{5pt}

**Harris County** and **Travis County** are both home to large cities, Houston and Austin, respectively, that are known for economic growth and diverse demographics. Their consistent clustering results indicate resilience and a high likelihood of sustained consumer activity, even during crises. The recommendation is to consider these counties for developments that leverage cultural and economic diversity, such as mixed-use spaces that cater to both residential and commercial needs. 

\vspace{5pt}

The recommendations from this comprehensive analysis are actionable and based on robust data modeling. By focusing on counties like **Tarrant, Bexar, Dallas, Harris, and Travis**, the stakeholder can make informed decisions that maximize economic opportunities while minimizing risk.

\newpage

# Conclusion
Our analysis of Texas counties using a combination of clustering techniques—including K-Means (both supervised and unsupervised), Hierarchical Clustering, and Gaussian Mixture Models (GMM)—sheds light on the critical role that economic status plays in determining public health outcomes during a pandemic. Wealthier counties, characterized by higher median incomes, lower rent burdens, and robust healthcare infrastructure, consistently show lower mortality rates, highlighting the tangible benefits of economic and healthcare resilience.

\vspace{5pt}

The models identified specific counties, such as **Tarrant, Bexar, Dallas, Harris, and Travis**, as high-performing regions, not only in terms of pandemic outcomes but also in their economic stability and potential for sustainable growth. These areas have strong population densities and urban infrastructure, which are crucial for supporting mixed-use development projects. The clustering results revealed that wealth and healthcare access are significant determinants of a county's ability to manage crises, making these counties ideal candidates for property development that requires consistent consumer activity and economic vibrancy.

\vspace{5pt}
**Implications for Stakeholders**

  - **Economic and Public Health:** The analysis underscores the direct correlation between economic prosperity and public health outcomes. Wealthier counties can leverage superior healthcare systems, effective public health measures, and greater social resources, leading to lower COVID-19 mortality rates. This insight is vital for developers and stakeholders who wish to mitigate risks associated with global health crises.
  - **Investment Opportunities:** For stakeholders, investing in counties with demonstrated economic resilience presents a low-risk, high-reward opportunity. **Tarrant, Bexar, Dallas, Harris, and Travis** counties, all marked as "good" by multiple clustering models, are positioned for long-term growth and stability. Developers should focus on these regions for projects like mixed-use buildings that benefit from high foot traffic and diverse economic activity.
  - **Addressing Vulnerabilities:** The analysis also sheds light on economically disadvantaged areas where high rent burdens and lower median incomes amplify public health vulnerabilities. While these areas pose higher risks, they also present opportunities for impactful investments that improve community resilience. By funding affordable housing projects, healthcare facilities, and social infrastructure, developers can contribute to the equitable growth of these regions, benefiting both the community and long-term investors.

\vspace{5pt}
The findings from this analysis emphasize the importance of understanding the intersection between economic conditions and public health. By leveraging insights gained from clustering models, stakeholders can identify strategic opportunities that maximize economic benefits while fostering community resilience. As the world continues to adapt to the aftermath of the COVID-19 pandemic, the balance between economic opportunity and social responsibility will be paramount. The path forward lies in thoughtful, data-informed investments that not only yield returns but also uplift and strengthen the communities they serve.
