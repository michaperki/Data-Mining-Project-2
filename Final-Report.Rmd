...
# load up from csv file

COVID_19_cases_TX <- read.csv("data/COVID-19_cases_TX.csv")

# Remove statewide unallocated

COVID_19_cases_TX <- COVID_19_cases_TX %>% filter(county_name != "Statewide Unallocated")

# get total population from census data
...
# make census data compatible with COVID_19_cases_TX
...
# Create a table for each county in Texas containing total_deaths, total_cases, total_population, cases per thousand, deaths per thousand

county_texas_total_data <- COVID_19_cases_TX |>
  group_by(county_name)|>
  summarize(total_deaths = max(deaths), total_cases = max(confirmed_cases)) |>
  left_join(census_data, by = "county_name") |>
  mutate(cases_per_k = (total_cases/total_population) * 1000, deaths_per_k = (total_deaths/total_population) * 1000) %>%
  select(-state)
  # remove unwanted "state" feature

```

```{r, echo = FALSE}
# Load up data using census.gov API
...
# Make data set from county name API call
...
nrow(content_area_sq_miles_data_frame) == nrow(county_texas_total_data)
```

```{r, echo = FALSE}
# Add area in square milage to prior county data set and calculate population density
county_data_set_with_pop_density <- county_texas_total_data %>%
                                    left_join(content_area_sq_miles_data_frame, by = "county_name") %>%
                                    mutate(pop_density = total_population/area_sqmiles)

# Scaled/Normalized Data
county_data_set_with_pop_density.scaled <- county_data_set_with_pop_density %>% mutate(across(where(is.numeric), ~ as.numeric(scale(.))))
```
...
```{r, echo=FALSE}
# Rename some data to make code work because I originally used wicked, evil, and confusing RDS files muahahaha

olivia_cluster_kmeans_data <- data_clustered_2
olivia_cluster_kmeans_two_var <- olivia_cluster_kmeans_data %>% select(county_name, cluster)
olivia_cluster_hierarchical_data <- data_clustered_hc
olivia_cluster_hierarchical_two_var <- olivia_cluster_hierarchical_data %>% select(county_name, cluster_hc)

county_data_set <- county_data_set_with_pop_density
county_data_set.scaled <- county_data_set_with_pop_density.scaled
```

```{r, echo=FALSE}

# Combine data to make tables we are going to use
county_data_set <- county_data_set %>% left_join(olivia_cluster_kmeans_two_var, by = "county_name") %>% left_join(olivia_cluster_hierarchical_two_var, by = "county_name") %>% rename(olivia_cluster_kmeans = "cluster") %>% rename(olivia_cluster_hc = "cluster_hc")

county_data_set.scaled <- county_data_set.scaled %>% left_join(olivia_cluster_kmeans_two_var, by = "county_name") %>% left_join(olivia_cluster_hierarchical_two_var, by = "county_name") %>% rename(olivia_cluster_kmeans = "cluster") %>% rename(olivia_cluster_hc = "cluster_hc")
```
...
The clustering seemingly divides counties by population density
...
```
  ...
  - **K-means Cluster 1: ** This cluster has a higher average cases and deaths per thousand with an average population density much lower to that of the second cluster. This could be attributed as a "COVID-19 low population density" cluster of counties.
  
  - **K-mean Cluster 2: ** This cluster has a lower average cases and deaths per thousand with an average population density similar to that of the first cluster. This could be attributed as a "COVID-19 high population" cluster of counties.
...
Our Average Silhouette width is not close to 1, which means that the centroids may not be as close to the middle of the cluster as they could be; however, the distribution of data points are fair.
  ...
  - **K-means Cluster 1:** With 95 points and an average Silhoutte plot of 0.77. The Cohesion is fair.
  - **K-means Cluster 2** With 6 points and an average Silhoutte plot of 0.39. This Silhouette plot could be interpreted as not very cohesive.
...
fviz_dend(pop_dense_and_cases_hierarchical, show_labels = FALSE, main = "Hierarchical Clustering based off Population Density and Cases\nper Thousand")
```


```{r, echo=FALSE}
fviz_dend(pop_dense_and_cases_hierarchical, k = 2, show_labels = FALSE, main = "Hierarchical Clustering based off Population Density and Cases\nper Thousand For Two Clusters")
```
...
```
  ...
  - **Hierarchical Cluster 2: ** This is a cluster of the three highest population density counties in the data set and cna be attributed as the "high population density" cluster.
...
Our average silhouette widths are close to 1, which means the centroids are close to the center of the clusters; however, the distribution of data points in the cluster are very lop sided in favor of the low population density cluster.
  ...
  - **Hierarchical Cluster 1:** With 136 points and an average Silhoutte plot of 0.81, this can be interpreted as a high degree of cohesion.
  - **Hierarchical Cluster 2** This Silhoutte plot has little data points, but a high average Silhouttee with. This can be interpreted as a fair amount of cohesion.
...
Since the model is clustering for various factors such as affluence, COVID performance, and population density, the truth feature of our model will be defined by taking the total sum of a set of weights multiplied by the normalized value of numerical features. Assuming each aspect we are clustering for holds an equal amount of importance to our client, about a third each, the truth feature is defined using the following amount of weight with the following features.
  ...
  - **Feature 1: Deaths per Thousand** 16.65% Weight
  - **Feature 2: Cases per Thousand** 16.65% Weight
  - **Feature 3: Median Income** 33.3% Weight
  - **Feature 4: Population Density** 33.3% Weight
  
After normalizing using a z-score distribution, since a lower standard deviation indicates a better performance for both cases and deaths per thousand, COVID performance features will be subtracted instead of summed.

Mathematically, if our truth feature is defined as $\beta$, with each normalized numeric feature defined as f and weight number defined as w, the formula would be:

\vspace{10pt}

$\beta = -w_1f_1 -w_2f_2 + w_3f_3 + w_4f_4$
...
Both methods of clustering divided the data according to low population density, $k_1$ and $h_1$, and high population density $k_2$ $h_2$.  

\vspace{5pt}

Since a client likely wants a subset of counties with a higher population density, to make more money serving more people, $k_2$ and $h_2$ could be given as a subsets of affluent, high population density group of counties.  

\vspace{5pt}

To test the "goodness" of our data, for each county in the subset of $K_2$ and $H_2$ counties, we will us the $\beta$ formula on these subsets using aforementioned features, rank the counties from highest to lowest "goodness", divide the set in two at the same magnitude as their methodology subset equivalents, and perform a purity calculation for supervised analysis.
...
```{r, echo = FALSE}
# Get subsets with appropriate features

# Subset K2 and H2
subset_K_2 <- summary_data_kmeans %>% filter(olivia_cluster_kmeans == 2) %>% select(county_name, deaths_per_k, cases_per_k, median_income, pop_density)
subset_H_2 <- summary_data_hierarchical %>% filter(olivia_cluster_hierarchical == 2) %>% select(county_name, deaths_per_k, cases_per_k, median_income, pop_density)

# Subset k1 and k2
subset_kmeans_1 <- summary_data_kmeans %>% filter(matias_kmeans == 1) %>% select(county_name, matias_kmeans) %>% rename(cluster = "matias_kmeans")
subset_kmeans_2 <- summary_data_kmeans %>% filter(matias_kmeans == 2) %>% select(county_name, matias_kmeans) %>% rename(cluster = "matias_kmeans")

# Subset h1 and h2
subset_hierarchical_1 <- summary_data_hierarchical %>% filter(matias_hierarchical == 1) %>% select(county_name, matias_hierarchical) %>% rename(cluster_hc = "matias_hierarchical")
subset_hierarchical_2 <- summary_data_hierarchical %>% filter(matias_hierarchical == 2) %>% select(county_name, matias_hierarchical) %>% rename(cluster_hc = "matias_hierarchical")

# Normalize the features
subset_K_2 <- subset_K_2 %>% mutate(across(where(is.numeric), scale))
subset_H_2 <- subset_H_2 %>% mutate(across(where(is.numeric), scale))

# Make "goodness" or beta calculation
subset_K_2 <- subset_K_2 %>% mutate(beta = -.1665 * deaths_per_k - .1665 * cases_per_k + .333 * median_income + .333 * pop_density)
subset_H_2 <- subset_H_2 %>% mutate(beta = -.1665 * deaths_per_k - .1665 * cases_per_k + .333 * median_income + .333 * pop_density)

# Subsets divided into two to equal to the same magnitude as subsets
subset_a_1 <- subset_K_2 %>% arrange(desc(beta)) %>% slice_tail(n = nrow(subset_kmeans_1))
# We slice head because top results are in k2
subset_a_2 <- subset_K_2 %>% arrange(desc(beta)) %>% slice_head(n = nrow(subset_kmeans_2))

subset_b_1 <- subset_H_2 %>% arrange(desc(beta)) %>% slice_tail(n = nrow(subset_hierarchical_1))
# We slice head because top results are in h2
subset_b_2 <- subset_H_2 %>% arrange(desc(beta)) %>% slice_head(n = nrow(subset_hierarchical_2))
...
rows_in_K2 <- nrow(subset_K_2)
rows_in_H2 <- nrow(subset_H_2)

purity_kmeans_cluster <- (1/rows_in_K2) * (max(length(intersect(subset_kmeans_1$county_name, subset_a_1$county_name)),
                                               length(intersect(subset_kmeans_1$county_name, subset_a_2$county_name))) +
                                           max(length(intersect(subset_kmeans_2$county_name, subset_a_1$county_name)),
                                               length(intersect(subset_kmeans_2$county_name, subset_a_2$county_name)))
)

purity_hierarchical_cluster <- (1/rows_in_H2) * (max(length(intersect(subset_hierarchical_1$county_name, subset_b_1$county_name)),
                                               length(intersect(subset_hierarchical_1$county_name, subset_b_2$county_name))) +
                                           max(length(intersect(subset_hierarchical_2$county_name, subset_b_1$county_name)),
                                               length(intersect(subset_hierarchical_2$county_name, subset_b_2$county_name)))
)
```
...
With a purity of 1 being a perfect match of clusters, both k and h have very high purities. The high purities in these subsets can be attributed to the magnitude of $k_2$ and $h_2$ being very low, with 6 and 3 counties respectively. If the vast majority of counties are in subset $k_1$ and $h_1$, of course there is going to be a high rate of purity. Therefore, a more interesting comparison for $k_2$ and $h_2$ would be just comparing them to their equivalent obtained using the truth feature $a_2$ and $b_2$, counting the number of times they intersect, and dividing by the magnitude of the subsets.

\vspace{10pt}

```{r, echo = FALSE}
comparison_for_k2 <- (1/6) * length(intersect(subset_kmeans_2$county_name, subset_a_2$county_name))
```


Subset $k_2$ consists of
...
The only difference in the two subsets are Travis and Bexar county are in subset $k_2$ while Fort Bend and Denton county are in $a_2$. Four of the six counties found through our clustering were found also found in our truth feature, which is 66.7%.

\vspace{5pt}

```{r, echo = FALSE}
comparison_for_h2 <- (1/3) * length(intersect(subset_hierarchical_2$county_name, subset_b_2$county_name))
```
...
Two of the three counties found through our clustering were found also found in our truth feature, which is 66.7%. 

Therefore, according to the truth feature, these clusters are both 66.7% accurate. (Using this metric makes more sense than purity because the low number of relevant results)
