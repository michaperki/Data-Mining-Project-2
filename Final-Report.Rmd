...
The clustering seemingly divides counties into either high cases per thousand or low cases per thousand.
...
Our Average Silhouette width is not close to 1, which means that the centroids may not be as close to the middle of the cluster as they could be; however, the distribution of data points are fair.
  ...
  - **K-means Cluster 1:** With 53 points and an average Silhoutte plot of 0.51. The Cohesion is fair.
  - **K-means Cluster 2** With 100 points and an average Silhoutte plot of 0.60. The Cohesion of this Silhouette plot could be interpreted as better than the prior cluster but not far better.
...
subset_a_1 <- subset_K_2 %>% arrange(desc(beta)) %>% slice_head(n = nrow(subset_kmeans_1))
subset_a_2 <- subset_K_2 %>% arrange(desc(beta)) %>% slice_tail(n = nrow(subset_kmeans_2))
